...
class LBSNData(Dataset):
    ...
    def build_vocab(self, data_name, data_path, min_loc_freq):
        if(data_name =="FourSquare"):
            data_path = "D:\研二\代码\STiSAN\FourSquare\FourSquare_POIs.txt"
        for line in open(data_path, encoding='utf-8'):
            ...
            if data_name == 'weeplaces':
                # user, loc, time(2010-10-22T23:44:29), lat, lng,city,cat
                loc = line[1]
                ...
                loc = line[2]
                coordinate = float(line[3]), float(line[4])
            elif data_name == 'NYC'or data_name == "TKY" :
                
                #user,loc,catID,catname,lat,lon,time_offset,time
                loc=line[1]
                coordinate = float(line[4]),float(line[5])
            elif data_name == 'FourSquare':
                loc = line[0]
                coordinate =float(line[1]),float(line[2])

            self.add_location(loc, coordinate)
        checkin_path="D:\研二\代码\STiSAN\FourSquare\FourSquare_Checkins.txt"
        for line in open(checkin_path, encoding='utf-8'):
            line = line.strip().split('\t')
            if(len(line)!=4):
                continue
            loc=line[1]
            self.countLocation(loc)
    ...
    def countLocation(self,loc):
        if loc in self.loc2idx:
    ...
    def processing(self, data_name, data_path, min_user_freq, map_level):
        ...
        n_user = 1
        if(data_name =="FourSquare"):
            data_path = "D:\研二\代码\STiSAN\FourSquare\FourSquare_Checkins.txt"
        for line in open(data_path, encoding='utf-8'):
            line = line.strip().split('\t')
            # line = line.strip().split(',')
            if len(line) != 4:
            ...
            # if len(line) != 5:
            #     continue
            # if  (len(line)!=7) or line[0]=='userid':
            #     continue
            if data_name == 'weeplaces':
                # user, loc, time(2010-10-22T23:44:29), lat, lng
                user, loc, t, lat, lng ,city,cat= line
                lat = float(lat)
                lng = float(lng)
                ...
                #user,loc,catID,catname,lat,lng,time_offset,time(Tue Apr 03 18:10:04 +0000 2012)
                user,loc,catID,catname,lat,lng,time_offset,t = line
                lat = float(lat)
                lng = float(lng)
                ...
                try:
                    user,loc,t,time_offset = line
                    timestamp =time.strptime(t,"%a %b %d %H:%M:%S +0000 %Y")
                    lat,lng = self.loc2gps[loc]
                except ValueError:
                    continue 
            if loc not in self.loc2idx:
                continue
    ...
    # def data_partition(self, max_len):
        ...
        for user in tqdm(range(len(self))):
            ...
            wenday_matrix, clock_matrix = st_matrix[user]
            ...
            #                                   temporal_matrix,
            #                                   spatial_matrix)
            w_mat, c_mat = extract_sub_matrix(max(0, i - max_len),
                                              i,
                                              len(eval_src_seq),
                                              max_len,
                                              wenday_matrix,
            ...
            # eval_seq.append((eval_src_seq, eval_trg,w_mat, s_mat,c_mat))
            ...
            for k in range(n_instance):
                if (i - k * max_len) > max_len * 1.1:
                    ...
                    #                                   temporal_matrix,
                    #                                   spatial_matrix)
                    w_mat, c_mat = extract_sub_matrix(i - (k + 1) * max_len - 1,
                                                      i - k * max_len - 1,
                                                      len(train_src_seq),
                                                      max_len,
                                                      wenday_matrix,
                    ...
                    train_seq.append((train_src_seq, train_trg_seq,w_mat,s_mat,c_mat))
                    ...
                    #                                   temporal_matrix,
                    #                                   spatial_matrix)
                    w_mat,c_mat = extract_sub_matrix(max(i - (k + 1) * max_len - 1, 0),
                                                      i - k * max_len - 1,
                                                      len(train_src_seq),
                                                      max_len,
                                                      wenday_matrix,
                                                      ...
                                                      clock_matrix)
                    # train_seq.append((train_src_seq, train_trg_seq, t_mat, s_mat))
                    train_seq.append((train_src_seq, train_trg_seq,w_mat, c_mat))
                    # train_seq.append((train_src_seq, train_trg_seq))
                    break
    ...
    def spatial_temporal_matrix_building(self, path):
        ...
        for u in tqdm(range(len(self))):
            ...
            wenday_matrix,clock_matrix = build_st_matrix(self, seq, len(seq))
            user_matrix[u] = (wenday_matrix,clock_matrix)
