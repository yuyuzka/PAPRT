...
def evaluate(model, max_len, eval_data, eval_sampler, eval_batch_size, eval_num_neg, quadkey_processor, loc2quadkey, device, num_workers):
    model.eval()
    loader = DataLoader(eval_data, batch_size=eval_batch_size, num_workers=num_workers,drop_last =True,
    ...
    with torch.no_grad():
        # for _, (src_locs_, src_quadkeys_, src_times_, t_mat_, g_mat_, trg_locs_, trg_quadkeys_, data_size,src_periods_) in enumerate(loader):
        # for _, (src_locs_, src_quadkeys_, src_times_,trg_locs_, trg_quadkeys_, data_size) in enumerate(loader):
        for _, (src_locs_, src_quadkeys_, src_times_, w_mat,c_mat,trg_locs_, trg_quadkeys_, data_size,src_periods_) in enumerate(loader):
            ...
            mem_mask = None
            key_pad_mask = None
            # output ,srcout = model(src_loc, src_quadkey, src_time, t_mat, g_mat, pad_mask, attn_mask,
            #                trg_loc, trg_quadkey, key_pad_mask, mem_mask, data_size,src_period)
            # output,srcout = model(src_loc, src_quadkey, src_time, pad_mask, attn_mask,
